...
impl Context {
    ...
    /// Retrieves a value from the context by key in a blocking manner and tries to downcast it to the desired type.
    ///
    ...
    ///
    /// ```
    /// let value: MyValue = context().load_blocking("my_key").unwrap();
    /// ```
