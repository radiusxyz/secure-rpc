pub mod rpc_url {
    pub const DEFAULT_RPC_URL: &str = "http://127.0.0.1:9000";
    pub const DEFAULT_TX_ORDERER_RPC_URL_LIST: &str = "http://127.0.0.1:3000";
    pub const DEFAULT_ROLLUP_RPC_URL: &str = "http://127.0.0.1:8123";
    pub const DEFAULT_DKG_RPC_URL: &str = "http://127.0.0.1:7100";
    pub const DEFAULT_BLOCKCHAIN_HTTP_URL: &str = "http://127.0.0.1:8545";
    pub const DEFAULT_BLOCKCHAIN_WS_URL: &str = "ws://127.0.0.1:8545";
    pub const DEFAULT_CONTRACT_ADDRESS: &str = "0x5FbDB2315678afecb367f032d93F642f64180aa3";
}
